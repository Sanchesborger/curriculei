...
export const HomeScreen: React.FC<HomeScreenProps> = ({
  ...
  return (
    <main className="pt-6 md:pt-8 pb-24 px-5 max-w-5xl mx-auto space-y-8 font-sans">
      ...
      <section className="block md:hidden font-sans space-y-3 pt-1">
        ...
        <div className="flex items-stretch gap-3 overflow-x-auto pb-3 pt-1 no-scrollbar [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden snap-x scroll-smooth -mx-5 px-5 scroll-px-5">
          {/* Card 1: Criar Currículo */}
          <button
            onClick={onCreateNewResume}
            className="w-[145px] sm:w-[155px] shrink-0 snap-start bg-[#004ac6] text-white p-4 rounded-2xl shadow-sm hover:bg-[#2563eb] transition-all flex flex-col justify-between text-left active:scale-95 cursor-pointer space-y-3 relative group border border-blue-500/30"
          >
          ...
          <button
            ...
            className="w-[145px] sm:w-[155px] shrink-0 snap-start bg-white text-[#191c1e] p-4 rounded-2xl border border-slate-200/90 shadow-2xs hover:border-[#2563eb] transition-all flex flex-col justify-between text-left active:scale-95 cursor-pointer space-y-3 group"
          >
            <div className="flex items-center justify-between w-full">
              <div className="p-2.5 bg-blue-50 text-[#004ac6] rounded-xl border border-blue-100">
              ...
              >
                <div>
                  <div className="flex items-center justify-between gap-2 mb-1.5">
                    <span className="text-[10px] font-bold text-white bg-white/20 backdrop-blur-xs px-2.5 py-0.5 rounded-full border border-white/20">
                      {job.type}
                    </span>
                    <span className="text-[10px] font-bold text-emerald-300 bg-emerald-950/40 px-2.5 py-0.5 rounded-full border border-emerald-400/40">
                      {job.matchPercentage}% Compatível
                    </span>
                  </div>

                  <h4 className="font-bold text-sm text-white group-hover:text-blue-100 transition-colors line-clamp-1">
                    {job.title}
                  </h4>

                  <div className="text-xs text-blue-100/90 font-medium space-y-1 mt-1.5">
                    <div className="flex items-center gap-1.5">
                      <Building2 className="w-3.5 h-3.5 text-blue-200 flex-shrink-0" />
                      <span className="truncate">{job.company}</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <Navigation className="w-3.5 h-3.5 text-blue-200 flex-shrink-0" />
                      <span className="text-emerald-300 font-bold">~ {job.distanceKm} km de você</span>
                    </div>
                  </div>
                </div>

                <div className="pt-2.5 border-t border-white/20 flex items-center justify-between text-xs mt-2">
                  <span className="font-bold text-white text-[11px]">{job.salary}</span>
                  <span className="text-[11px] font-bold text-blue-100 group-hover:text-white flex items-center gap-0.5 group-hover:translate-x-0.5 transition-transform">
