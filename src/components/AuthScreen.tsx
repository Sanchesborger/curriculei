...
export const AuthScreen: React.FC<AuthScreenProps> = ({
  ...
  // Social Login Modal state for Google / Apple registration
  ...
  const handleGoogleAuth = async () => {
    ...
    // Open Google Account modal with completely empty fields
  ...
  const handleAppleAuth = async () => {
    ...
    // Open Apple Account modal with completely empty fields
  ...
  const handleConfirmSocialModal = async (e?: React.FormEvent) => {
    ...
    if (!socialEmail || !socialEmail.includes('@')) {
      alert('Por favor, informe um e-mail válido para cadastrar sua conta ' + (socialModalProvider === 'google' ? 'Google' : 'Apple') + '.');
      return;
    }

    setIsLoading(true);
    const provider = socialModalProvider || 'google';
          ...
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border border-[#c3c6d7]/60 flex flex-col gap-5 relative animate-in zoom-in-95 duration-200">
            ...
            <div className="flex flex-col items-center text-center gap-2 pt-2">
              ...
              <p className="text-xs text-[#737686]">
                Cadastre seus dados para sincronizar e salvar sua nova conta na base de dados do Supabase.
              </p>
            </div>
            ...
            <form onSubmit={handleConfirmSocialModal} className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-semibold text-[#434655]">
                  E-mail da Conta {socialModalProvider === 'google' ? 'Google' : 'Apple'}
                </label>
                <input
                  type="email"
                  required
              ...
              <div className="flex flex-col gap-1 text-[11px] text-gray-600 bg-blue-50/80 p-3 rounded-xl border border-blue-100">
                <div className="flex items-center gap-1.5 font-semibold text-blue-900">
                  <ShieldCheck className="w-4 h-4 text-blue-600 shrink-0" />
                  <span>Sincronização com Banco Supabase</span>
                </div>
                <p className="text-[#434655] leading-relaxed">
                  Sua conta será registrada no banco de dados PostgreSQL do Supabase.
                </p>
                <p className="text-[10px] text-gray-500 mt-1 pt-1 border-t border-blue-200/60">
                  💡 <strong>Nota para Autenticação OAuth Direta:</strong> Para login direto sem preenchimento, insira o <em>Client ID</em> do Google no painel do Supabase (<em>Authentication &gt; Providers &gt; Google</em>).
                </p>
              </div>
              ...
              >
                <span>{isLoading ? 'Cadastrando no Supabase...' : `Entrar com ${socialModalProvider === 'google' ? 'Google' : 'Apple'}`}</span>
                <ArrowRight className="w-4 h-4" />
              </button>
            </form>
          </div>
